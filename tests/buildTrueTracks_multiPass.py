...
This modified version imagines that we have several layes of tracking.
...
def trackStartTime(objectDets, track):
    return min(allTrackObsTimes(objectDets, track))


def allTrackObsTimes(objectDets, track):
    res = set()
    for tracklet in track:
        for d in tracklet:
            res.add(objectDets[d].time)
    return res


def linkTracks(objectDets, tracks, linkTracksTimeWindow, 
                   longTrackMinObs, longTrackMinNights):
    outTracks = []
    if (len(tracks)) > 1:
        trackStartTimes = []
        for track in tracks:
            #print "got track: ", track
            trackStartTimes.append(trackStartTime(objectDets, track))

        trackStartTimes = sorted(trackStartTimes)
        #print "Got track start times are ", trackStartTimes
        for i in range(len(trackStartTimes)):
            DEBUG = False
            startTrack = tracks[i]
            longTrackStartTime = trackStartTime(objectDets, startTrack)
            curLongTrack = [startTrack]
            trackObsTimes = allTrackObsTimes(objectDets, startTrack)

            for j in range(i + 1, len(trackStartTimes)):
                jTime = trackStartTime(objectDets, tracks[j])
                if jTime - longTrackStartTime < linkTracksTimeWindow:
                    if DEBUG:
                        print jTime, longTrackStartTime, " separation = ", jTime - longTrackStartTime
                        print "longTracksTimeWindow = ", linkTracksTimeWindow
                    curLongTrack.append(tracks[j])
                    jTrackTimes = allTrackObsTimes(objectDets, tracks[j])
                    if DEBUG:
                        print "JTrack obs times were ", jTrackTimes
                        print "JTrack trackStartTime was ", jTime
                    trackObsTimes = trackObsTimes.union(jTrackTimes)
                    if DEBUG:
                        print "after merged-in track times are ", trackObsTimes
                else:
                    if DEBUG:
                        print jTime, longTrackStartTime, " separation = ", jTime - longTrackStartTime
                        print "longTracksTimeWindow = ", linkTracksTimeWindow

            obsTimesList = list(trackObsTimes)
            if DEBUG:
                print "Got long track with detections at ", sorted(obsTimesList)
                print "Which are nights ", set(map(int, obsTimesList))

            if isFindableFromTimes(obsTimesList, 
                                   minPerNight=1, 
                                   minNights=longTrackMinNights):
                if DEBUG:
                    print "Got track is findable"
                outTracks.append(curLongTrack)
            else:
                if DEBUG:
                    print "Got track is NOT findable."
                    print "With nights ", set(map(int, obsTimesList))
                pass
...
def writeTracks(objectDets, velocityLimit, trackletTimeLimit,
                ...
                shortTrackMinObs, shortTrackMinNights,
                linkTracksTimeWindow, longTrackMinObs, longTrackMinNights):
    ...
    tracks = linkTracklets(objectDets, tracklets, raAccelerationLimit, decAccelerationLimit,
                           linkTrackletsTimeWindow, shortTrackMinObs, shortTrackMinNights)
    if DEBUG:
        print "got short tracks: ", tracks
        if (len(tracks) == 0):
            print "Could not find object with dets: "
            for det in objectDets:
                print det.time, det.ra, det.dec

    longTracks = linkTracks(objectDets, tracks, linkTracksTimeWindow, 
                            longTrackMinObs, longTrackMinNights)
    ...
    return len(longTracks), trackletStartTimes, curTrackId
...
if __name__ == "__main__":
    ...
    shortTrackMinObs, shortTrackMinNights = map(int, sys.argv[6:8])
    linkTrackletsTimeWindow = float(sys.argv[8])
    longTrackMinObs, longTrackMinNights = map(int, sys.argv[9:11])
    linkTracksTimeWindow = float(sys.argv[11])
    [infile, tracksOutfile, findableObjects] = sys.argv[12:]
    ...
    print "Min obs for short track:                  %d" % shortTrackMinObs
    print "Min nights for short track:               %d" % shortTrackMinNights
    print "Got tracklet linking time window:         %f" % linkTrackletsTimeWindow
    print "Min obs for long track:                   %d" % longTrackMinObs
    print "Min nights for long track:                %d" % longTrackMinNights
    print "Got track/track linking time window:      %f" % linkTracksTimeWindow
    ...
    while line != []:
        ...
        if done:
            ...
            nTracks, trackletStartTimes, curTrackId = writeTracks(curObjectData, velocityLimit, trackletTimeLimit,
                                                                  ...
                                                                  curTrackId, shortTrackMinObs, shortTrackMinNights,
                                                                  linkTracksTimeWindow, longTrackMinObs, longTrackMinNights)
